...
if ($stmt->execute()) {
    ...
    $appointment_hash = generarAlfanumerico();

    $stmt2 = $conn->prepare(
        "INSERT INTO hub_crm_appointments (id_lead, info_appointment, appointment_status, created_appointment, appointment_type, appointment_hash)
        VALUES (?, ?, ?, ?, ?, ?)"
    );
...
function generarAlfanumerico(int $length = 12): string
{
